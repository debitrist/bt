...
symbol = ['^vix','spy']
symbol.sort()
bars = web.DataReader(symbol, 'yahoo', datetime.datetime(1995, 1, 1), datetime.datetime(2019, 2, 24))

returndays = (1,2,3,4,5,10,15,20,25,30,50,100)
returndayindex=["D"+str(i) for i in returndays]

macro = web.DataReader(['BAMLC0A1CAAAEY','BAMLC0A4CBBBEY','BAMLH0A1HYBBEY','BAMLH0A3HYCEY'], 'fred', datetime.datetime(1995, 1, 1), datetime.datetime(2019, 2, 24))
macro=macro.fillna(method='bfill')
macro['HYspread']=macro['BAMLC0A4CBBBEY']-macro['BAMLC0A1CAAAEY']
bars['spreadyoy','macro']=macro['HYspread']/macro['HYspread'].shift(250)-1
bars['spreadzma','macro']=(macro['HYspread']-macro['HYspread'].rolling(250).mean())/macro['HYspread'].rolling(250).std()
...
def sortbinsmean (cutobj, ticker='spy', bincount=10):
...
creditgrp = sortbinsmean(bars['spreadyoy','macro'])
creditzmagrp=sortbinsmean(bars['spreadzma','macro'])
...
tabgrp(rgrp, 'rvalue')
tabgrp(creditgrp,'credit')
tabgrp(creditzmagrp,'creditz')
